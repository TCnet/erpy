class InoutplansController < ApplicationController
  ...
  def index
    @inoutplans = current_warehourse.inoutplans.order(created_at: :desc).paginate(page: params[:page])
  end

  def destroy
    #出库
    @inoutplan = Inoutplan.find(params[:id])
    out_stock(@inoutplan,current_warehourse)
    @inoutplan.destroy
  ...
  def update
    ...
    out_stock_str(@inoutplan.inout,current_warehourse,@inoutplan.presku.to_s)
    @inoutplan.inoutstocks.each do |d|
      d.delete
    end

    if @inoutplan.update(inoutplan_params)
      ...
      in_stock_str(inouts,current_warehourse,presku)
      #in_stock(@inoutplan,current_warehourse)
      #in_stock(@inoutplan,current_warehourse)
  ...
  def importexcel
    @inoutplan = Inoutplan.find(params[:id])
    #删除之前记录
    out_stock_str(@inoutplan.inout,current_warehourse,@inoutplan.presku)
    ...
    in_stock_str(@inoutplan.inout,current_warehourse,@inoutplan.presku)
  ...
  def create
    @inoutplan = Inoutplan.new(inoutplan_params)
    @inoutplan.user =current_user
    @inoutplan.warehouse = current_warehourse
    ...
    if @inoutplan.save
      ...
      #in_stock(@inoutplan,current_warehourse)
      in_stock_str(inoutplan_params["inout"],current_warehourse,inoutplan_params["presku"])
